import {
  FETCH_ALL_MEDIAS_REQUEST,
  FETCH_ALL_MEDIAS_SUCCESS,
  FETCH_ALL_MEDIAS_FAILURE,
  FETCH_MEDIA_SUCCESS,
  FETCH_MEDIA_SUCCESS2,
  FETCH_MEDIA_SUCCESS3,
  FETCH_MEDIA_REQUEST,
  FETCH_MEDIA_FAILURE,
...
export const mediasReducer = (state = initialState, { type, payload }) => {
  switch (type) {
    ...
    case FETCH_ALL_MEDIAS_SUCCESS:
      ...
      var anime = payload.filter(
        (payload) => payload.original_language === "ja",
      );
      var english = payload.filter(
        (payload) => payload.original_language === "en",
      );
      var regional = payload.filter(
        (payload) =>
          payload.original_language === "ml" ||
          payload.original_language === "ta" ||
          payload.original_language === "hi",
      );
      tv.length = 15;
      movie.length = 15;
      anime.length = 15;
      ...
      for (let i = 0; i < payload.length; i++) {
        if (payload[i].type === "main") carmain.push(payload[i]);
      }
      // console.log("AA", carmain,tv,movie,anime,english,regional)
      const media1 = [...payload].sort(
    ...
    case FETCH_MEDIA_SUCCESS: {
      ...
      for (let i = 0; i < payload.genre_ids.length; i++) {
    ...
    case FETCH_MEDIA_SUCCESS3: {
      ...
      for (let i = 0; i < payload.episodes.length; i++) {
        var temp = {
          ...
          image:
            "https://image.tmdb.org/t/p/w500" + payload.episodes[i].still_path,
